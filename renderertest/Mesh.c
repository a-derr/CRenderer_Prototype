...
void load_obj_file_data(char* filename) {
	...
	while (fgets(line, 1024, file)) {
		if (strncmp(line, "v ", 2) == 0) {
			vec3_t line_vertices = {0, 0, 0};
			sscanf_s(line, "v %f %f %f", &line_vertices.x, &line_vertices.y, &line_vertices.z);
			array_push(mesh.vertices, line_vertices);
		}

		if (strncmp(line, "vt ", 3) == 0) {
			tex2_t line_uvs = { 0, 0 };
			sscanf_s(line, "vt %f %f", &line_uvs.u, &line_uvs.v);
			array_push(texcoords, line_uvs);
		}
